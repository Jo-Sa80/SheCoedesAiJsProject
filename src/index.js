function generateRecipe(event) {
  ...
  recipe.innerHTML = `please wait, while your recipe is created <span class="blink">...<span>`;
  ...
  let context =
    "You are a great cook. Tell me the name of the Recepie in <strong> with a <br/> after and the display only the main ingredients with the quantity. please display every ingredient in a separate line with a br/ in between, starting with the first ingredient.";
  let prompt = `Please give me a short recipe for ${ingredientElement.value} and follow the instructions.`;

  let apiUrl = `https://api.shecodes.io/ai/v1/generate?prompt=${prompt}&context=${context}&key=${apiKey}`;

  let resultElement = document.querySelector("#hidden");
